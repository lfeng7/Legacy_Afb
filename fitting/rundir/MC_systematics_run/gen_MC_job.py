...
for i in range(PDF_members) :
	...
	cmd = 'cp standard_grid_run temp_'+str(i)+' -r \n'
	#cmd += 'cp ana.listOfJobs tmp_'+str(i) 
	#print cmd 
        os.system(cmd)
	#Commmand to run all jobs
	run_MC_cmd += 'cd temp_'+str(i)+'\nsource grid_sub_MC.csh\ncd ..\n'
        run_data_separated_cmd += 'cd temp_'+str(i)+'\nsource grid_sub_separated.csh\ncd ..\n'
        run_data_combined_cmd += 'cd temp_'+str(i)+'\nsource grid_sub_combined.csh\ncd ..\n'
	#Command to clean MC or data grid files
	clean_MC_cmd += 'cd temp_'+str(i)+'\nsource cleanup_MC.sh\ncd ..\n'
        clean_data_cmd += 'cd temp_'+str(i)+'\nsource cleanup_grid.sh\ncd ..\n'
